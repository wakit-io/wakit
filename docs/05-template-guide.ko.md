...
결과적으로, 커스텀 템플릿에서 자체 localStorage 키(예: `'app_astro-theme'`)로만 테마를 저장하면 앱 재시작 시 wakit이 덮어써서 다크 모드가 풀립니다.
...
```bash
npm run package:app_astro   # 빌드 + 스키마 포함 ZIP 자동 생성
```

결과물: `packages/hybrid-ui-template-app_astro-v1.0.0.zip`
